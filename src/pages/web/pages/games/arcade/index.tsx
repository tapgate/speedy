...
class ArcadeGameMode {
  ...
  updateData(data: IGameData) {
    ...
    this.spawnTileSpeed = (1 / data.speed) * 100;
  }
}
...
const ArcadeGame = () => {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (gameMode.lives > 0) {
        ...
        if (map) {
          gameModeRef.current.feet += gameMode.speed * (timeDelta / 1000);
          ...
          setTiles((prev) => {
            ...
            for (let i = 0; i < tileIds.length; i++) {
              ...
              if (tile) {
                tile.x -= gameMode.speed * 25 * (timeDelta / 1000);
  ...
  const jump = () => {
    ...
    if (character && jumpArea) {
      ...
      const jumpCieling = jumpArea.offsetTop;

      const jumpSpeed = gameMode.jumpSpeed * 2 * pixleSize;
            ...
            <div
              ...
              style={{
                ...CSSDimensionsWithPixelSize(
                  `${gameMode.tileWidth}px`,
                  `${gameMode.tileHeight}px`
                ),
          ...
          {gameMode.lives > 0 ? (
            <div className="text-center">
              {gameMode.jumpCount <= 0
                ? !isJumping && <div className="text-indigo">TAP TO JUMP</div>
                : ''}
            </div>
          ) : (
            <div className="text-center">
