...
export const ArcadeGame = ({ outfit, data, quit }: IArcadeGameProps) => {
  ...
  const spawnObstacleIfPossible = () => {
    ...
    if (canSpawnObstacle && game.state === ArcadeGameStateEnum.PLAYING) {
      ...
      setMapObjects((prev) => {
        ...
        obstacle.x = 100;
        obstacle.width = 8;
        obstacle.height = Math.floor(Math.random() * (32 - 8) + 8);

        obstacle.speed = game.speed * 10;
        ...
        obstacle.children = [0, 0, 0].map((_, i) => {
          ...
          point.x = 100;
          point.offsetX = 10 + (obstacle.width * pixleSize) / 2 + offset * pixleSize;
          point.width = 9 * (3 / (i + 1));
          point.height = 4;
    ...
    if (canSpawnCloud) {
      ...
      if (spawn) {
        setMapObjects((prev) => {
          ...
          newObject.value = Math.floor(Math.random() * (3 - 1) + 1);
  ...
  const jump = () => {
    ...
    const jumpSpeed = game.jumpSpeed * 300;
    ...
    if (character && jumpArea) {
      ...
      const jumpAreaPosition = {
        y: jumpArea.offsetTop
      };
  ...
  return (
    <>
      <div className={`scene relative w-full h-full overflow-hidden`} ref={sceneRef}>
        ...
        <div className="objects absolute top-0 left-0 right-0 z-10 h-[60%] bg-blue-400">
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      left: `calc(${mapObject.x}% + ${mapObject.offsetX ?? 0}px)`,
                      animationDuration: `${mapObject.speed * speedDampening}s`,
                      // maxWidth: 'calc(8vw + 128px)',
                      // maxHeight: 'calc(8vh + 64px)',
                      ...CSSDimensionsWithPixelSize(`${mapObject.width}px`, `${mapObject.height}px`)
                    }}>
                    <img
                      className="w-full pixelated"
                      src={`images/clouds/cloud-${padStart(`${mapObject.value ?? 1}`, 3, '0')}.png`}
                      alt=""
                    />
                  </div>
                );
              ...
              if (mapObject.type == MapObjectTypeEnum.OBSTACLE) {
                return (
                  <div
                    ...
                    <div
                      id={mapObject.id}
                      className="map-object obstacle bg-white border-2 border-black rounded-t-md"
                      style={{
          ...
          <div className="triggers absolute inset-0 flex items-end">
            <div className="absolute w-full h-full" style={{ top: 'calc(100% - 25px)' }}>
              <div className="relative w-full h-full">
                {Array.from(mapObjects.values()).map((mapObject) => {
                  if (mapObject.type == MapObjectTypeEnum.TRIGGER) {
                    return (
                      <div
                        key={mapObject.id}
                        title={`speed-dampening: ${speedDampening}`}
                        className={`absolute animated-object map-object bonus-points`}
                        style={{
                          top: '0',
                          left: `${mapObject.x}%`,
                          ...CSSDimensionsWithPixelSize(`${mapObject.width}px`, `100vh`),
                          transform: `translateX(calc(1px * var(--pixel-size) + ${
                            mapObject.offsetX ?? 0
                          }px))`,
                          animationDuration: `${(mapObject.speed ?? 50) * (speedDampening * 1.25)}s`
                        }}>
                        <div
                          id={mapObject.id}
                          className={`w-full h-full border-2 border-black bg-${mapObject.color}-500 mx-1 rounded-md`}>
                          <div className="relative w-full h-full">
          ...
          <div className="items absolute inset-0">
            {Array.from(mapObjects.values()).map((mapObject) => {
              if (mapObject.type == MapObjectTypeEnum.ITEM) {
                return (
                  <div
                    ...
                    id={mapObject.id}
                    title={`speed-dampening: ${speedDampening}`}
                    className={`absolute animated-object map-object item trigger-${mapObject.color} bg-${mapObject.color}-500 border-2 border-black rounded-lg`}
                    style={{
                      top: `calc(${mapObject.y}% + ${mapObject.offsetY ?? 0}px)`,
                    ...
                    <div
                      ...
                      <div style={{ fontSize: '2em' }}>
                        {mapObject.itemType == MapItemTypeEnum.HEALTH && <Icon name="HeartSolid" />}
          ...
          <div
            className="player relative w-full h-full flex justify-center items-end text-white font-black text-xl"
            ref={jumpAreaRef}>
            <div
        ...
        {/* overlay */}
        <div className="absolute overlay hidden animate__animated inset-0 z-10 bg-cyan-700/50 backdrop-blur-sm"></div>
        ...
        {game.state == ArcadeGameStateEnum.STARTING && (
          <div
            ...
            <div className="w-full flex flex-wrap justify-center">
              ...
              <div
                className="w-1/2 md:w-1/5 text-lg md:text-5xl font-semibold text-center p-2 md:p-4 bg-green-400 text-white rounded-lg border-4 border-green-600 cursor-pointer hover:opacity-75 active:opacity-50"
                onClick={() => tap()}>
                PLAY
              </div>
            </div>
